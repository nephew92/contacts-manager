...
class CustomArray extends Array {
  ...
  toJSON() {
    ...
    const self = [...this]
      .filter((item, idx, self) => item && item.valueOf() && self.findIndex((firstOcur) => firstOcur.valueOf().valueOf() === item.valueOf().valueOf()) === idx)
  ...
  isSame(other, bicheck = true) {
    for (let anotherItem of other) {
      if (!this.find(item => item && item.valueOf().valueOf() === anotherItem.valueOf().valueOf()))
        return false
    }
